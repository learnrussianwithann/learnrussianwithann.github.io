...
function getSpriteWithText(prop) {
	...
	s.name = 'sprite';
	if (prop.hasOwnProperty('name')) out.name = prop.name;
	if (prop.hasOwnProperty('textScale')) t.scale.set(prop.textScale);
	if (prop.hasOwnProperty('spriteScale')) s.scale.copyFrom(prop.spriteScale);
...
function getButton(prop) {
	...
	let bwidth = prop.hasOwnProperty('bwidth') ? prop.bwidth : 1;
	let bheight = prop.hasOwnProperty('bheight') ? prop.bheight : prop.hasOwnProperty('bwidth') ? prop.bwidth : 1;
	let out = new PIXI.Container();
	let t = new PIXI.Text(prop.text, prop.style);
	t.anchor.set(0.5);
